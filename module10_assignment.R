...
setwd("C:/Users/glady/OneDrive/Documents/R");
...
ui <- fluidPage(
  # App Title
  titlePanel("Module 10 Assignment"),#---1---
  # Page Layout
  sidebarLayout(
    # Side Panel
    sidebarPanel(
      # Interactive piece 1: inputID = "subset_income"
      selectInput(inputId="subset_income",label="subset income",choices = c("<=50K","50K")),
      # Interactive piece 2: inputId = "set_yaxis"
      selectInput(inputId="set_yaxis",label="set yaxis",choices = c("hours_per_week","capital_loss")),
      # Interactive piece 3: inputId = "subset_occupation"
      checkboxGroupInput(inputId = "subset_occupation",label="subset occupation",choices=unique(income$occupation),
...
server <- function(input, output) {
  ...
  create_subset <- reactive(income %>% 
                            filter(capital_loss > 0 & income== input$subset_income & occupation %in% input$subset_occupation))
  
  # Render Plot
  output$myfigure <- renderPlot(ggplot(create_subset()) +
                                  # Boxplot of x = occupation, y = defined by input
                                  geom_boxplot(aes(x="occupation",y=input$set_yaxis)) +
                                  theme_bw(18) +
